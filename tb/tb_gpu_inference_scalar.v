...
module tb_gpu_inference_scalar;
    localparam integer ROM_WORDS = 2048;
    localparam integer DMEM_WORDS = 2048;
    localparam integer OUTPUT_BASE_WORD = 353;
    ...
    initial begin
        ...
        $readmemh("demo/mlp16x16x4_scalar_program.hex", rom);
        $readmemh("demo/mlp16x16x4_scalar_data.hex", dmem);
        ...
        if (!halted) begin
            failures = failures + 1;
            $display("FAIL scalar program did not halt within cycle budget");
        end
        ...
        for (out_idx = 0; out_idx < 4; out_idx = out_idx + 1) begin
            ...
            if (dmem[OUTPUT_BASE_WORD + out_idx] !== expected[out_idx]) begin
                failures = failures + 1;
                $display(
                    "FAIL scalar output%0d got=%0d exp=%0d",
                    out_idx,
        ...
        if (failures == 0) begin
            $display("PASS tb_gpu_inference_scalar (%0d checks, cycles=%0d, fetched=%0d)", tests, cycles, fetched);
        end else begin
            $display(
                "FAIL tb_gpu_inference_scalar (%0d failures out of %0d checks, cycles=%0d, fetched=%0d)",
                failures,
                tests,
                cycles,
                fetched
